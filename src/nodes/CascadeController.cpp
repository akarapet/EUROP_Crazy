	...
	switch (yaml_controller_method)
	{
		case CONTROLLER_METHOD_RATES:
		{
			if (integrator_flag == INTEGRATOR_FLAG_ON)
			{
				...
				int_x       += yaml_integratorGain_forTauXY  * stateErrorBody[1] / yaml_control_frequency;
				int_y       += yaml_integratorGain_forTauXY  * stateErrorBody[0] / yaml_control_frequency;
				int_z       += yaml_integratorGain_forTauYaw * stateErrorBody[8] / yaml_control_frequency;
			}
